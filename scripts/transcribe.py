...
def main():
    ...
    # Auto-populate consul_key when --wait consul is specified
    if args.wait == "consul" and not args.consul_key:
        args.consul_key = f"services/video-transcription/{args.input_s3_path.split('/')[-1]}"
        # For better job identification, we'll use a simpler approach
        # We'll use the job ID that will be generated by the server
        # But since we don't have it yet, we'll just set a placeholder
        # The server will handle the actual standardization
        print(f"Auto-setting consul_key to: {args.consul_key}")
